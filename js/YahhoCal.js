...
YahhoCal.render = function(inputId, monthId, dateId) {
	...
	try{
		if(document.getElementById("language")){
			...
			this.locale=document.getElementById("language").value;
    ...
    //アダプタを取得
    //if (!this.adapters[currentId]) {//coment by chenbp 
...
YahhoCal.createCalendar = function(insertId) {
    ...
    yDom.setStyle(place, "z-index", 99999999);
    ...
    cal.selectEvent.subscribe(function(eventName, selectedDate) {
        ...
        YahhoCal.adapters[YahhoCal.currentId].setDate(date[0], date[1], date[2]);
 		...
 		var my_id=YahhoCal.currentId;
 		
 		if(   (my_id=='start_date')||(my_id=='end_date') 
 			||   (my_id=='start_time')||(my_id=='end_time')  )
 		{
 			judgeRoomUsage(my_id);
 		}else{
